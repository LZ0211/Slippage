...
class Fitting:
    ...
    def fit_leastsq(self):
        ...
        def cal_err(p,x,y):
            ...
            if self.locked[3]:
                s2 = self.params[3]
            _pos = interp1d(pos[0]*w1-s1,pos[1], fill_value="extrapolate")(x)
            _neg = interp1d(neg[0]*w2-s2,neg[1], fill_value="extrapolate")(x)
            return _pos-_neg-y
    ...
    def diff_fit_leastsq(self):
        ...
        def cal_err(p,x,y):
            ...
            if self.locked[3]:
                s2 = self.params[3]
            _pos = interp1d(pos[0]*w1-s1,pos[1], fill_value="extrapolate")(x) / w1
            _neg = interp1d(neg[0]*w2-s2,neg[1], fill_value="extrapolate")(x) / w2
            return _pos-_neg-y
        return least_squares(cal_err, params, args=full,verbose=1,bounds=bounds)
